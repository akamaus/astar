...
import aima.core.search.adversarial.{AlphaBetaSearch, Game}
import java.util
...
class Gobbler {
  ...
  class GobblerGame extends Game[State, Action, Int /* 0 or 1 */] {
    def getInitialState: State =
      State(Action.allCoordinates.map(x => x -> Square(List.empty)).toMap, 0)

    def getPlayers: Array[Int] = Array(0, 1)

    def getPlayer(state: State): Int = state.nextPlayer
    ...
    def getUtility(state: State, player: Int): Double = {
      winner(state) match {
        case (false, false) => throw new IllegalArgumentException("game is not terminal")
        case (true, false) => if (player == 0) 1 else -1
        case (false, true) => if (player == 1) 1 else -1
        case (true, true) => 0
      }
    }
  ...
  def go() {
    ...
    val ab = AlphaBetaSearch.createFor(gg)
